...
INSTRUCTION_PROMPT = """You are given a goal task instruction to accomplish, an observation from the envrionment, and previous actions you have taken (up to 4 past steps).
The observation is screen description parsed from the Android view hierarchy and contains the numeric tags and relevant informations (e.g., descriptions) of each UI element.
...
Please never take actions besides the options provided.
"""
...
Each observation from the environment and corresponding action taken by the expert is descripted as:
