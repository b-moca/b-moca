...
class DemonstrationStorer(RolloutStorer):
    ...
    def add(self, timestep, demo_act=None, vh_file_name=None):
        if self._current_episode != "": self._current_episode += "\n"
        ...
        # o_t+1
        timestep.curr_obs['text'], parsed_curr_obs, _ = parse_obs(timestep.curr_obs['text'],
                                                                  skip_non_leaf=False,
    ...
    def store(self, fn=None):
        if fn is None:
            fn = self._store_dir + "/" + datetime.datetime.now().strftime('%Y%m%d%H%M%S') + ".txt"
...
class TextActionParsingWrapper(base_wrapper.BaseWrapper):
    ...
    def step(self, raw_action, bbox_list):
        ...
        return BMocaTimeStep(env_id=self._env.curr_env_id,
...
def _collect_reward(timestep) -> bool:
  ...
  # print('Reward:', timestep.curr_rew)
        ...
        with load_env(args, task_name) as env:
            ...
            for target_env_id in _ENV_ID_LIST:
                ...
                while True:
                    ...
                    _, parsed_obs, parsed_bbox = parse_obs(raw_obs,
                                                           skip_non_leaf=False,
                                                           attribute_check=True)
                ...
                # store
                storer.store(fn=build_file_name(args, target_env_id, task_instruction, timenow))
...
def parse_args():
    ...
    # log
    parser.add_argument('--log_dir', type=str, default=f'{_WORK_PATH}/asset/dataset/demonstrations_text_action')
